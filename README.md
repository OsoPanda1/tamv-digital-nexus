...
### Inicio operativo de absorción evolutiva (Top 100 GitHub)

Se activó el arranque funcional del proceso de absorción para el perfil `OsoPanda1`:
- El scanner ahora acepta `targetUser`, `topRepos` (default 100) e `includePlan`.
- Con token GitHub (`GITHUB_API_TOKEN`) usa `user/repos` para incluir repos privados autorizados y colaboración/organización.
- Devuelve `topRepos` priorizados + `absorptionPlan` (acciones de integración seguras: escaneo, mapeo a canon, candidatos de parche, revisión humana).
