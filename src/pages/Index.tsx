...
const UNIFIED_STATS = [
  { value: "30+", label: "Formas Monetización", icon: <Wallet className="w-5 h-5" />, color: "aqua" as const },
  { value: "7", label: "Capas Federadas", icon: <Layers className="w-5 h-5" />, color: "purple" as const },
  { value: "∞", label: "DreamSpaces", icon: <Sparkles className="w-5 h-5" />, color: "gold" as const },
  { value: "100%", label: "Soberanía Digital", icon: <Shield className="w-5 h-5" />, color: "aqua" as const },
];
...
const QUICK_ACCESS_LINKS = [
  { label: "Metaverso", icon: <Globe className="w-4 h-4" />, path: "/metaverse", iconClass: "bg-cyan-500/20 text-cyan-300" },
  { label: "Isabella AI", icon: <Brain className="w-4 h-4" />, path: "/isabella", iconClass: "bg-purple-500/20 text-purple-300" },
  { label: "Universidad", icon: <GraduationCap className="w-4 h-4" />, path: "/university", iconClass: "bg-amber-500/20 text-amber-300" },
  { label: "DreamSpaces", icon: <Sparkles className="w-4 h-4" />, path: "/dream-spaces", iconClass: "bg-pink-500/20 text-pink-300" },
  { label: "Marketplace", icon: <ShoppingBag className="w-4 h-4" />, path: "/monetization", iconClass: "bg-emerald-500/20 text-emerald-300" },
  { label: "Gobernanza", icon: <Crown className="w-4 h-4" />, path: "/governance", iconClass: "bg-yellow-500/20 text-yellow-300" },
];
...
const LiveGrid = () => (
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
    {LIVE_CHANNELS.map((ch) => (
      ...
      >
        <img src={ch.img} alt="" className="w-full aspect-video object-cover" loading="lazy" crossOrigin="anonymous" />
            ...
            >
              <PremiumCard className="p-8 h-full">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-foreground">Tendencias Globales</h3>
                    <p className="text-sm text-muted-foreground">Lo más viral en TAMV</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {TRENDING_HASHTAGS.map((item, index) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span className="text-lg font-bold text-muted-foreground/30">0{index + 1}</span>
              ...
              <PremiumCard className="p-8 h-full">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Navega el ecosistema</p>
                  </div>
                </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            {NEXUS_COMMAND_DECK.map((item) => (
              <div
                key={item.title}
                className="relative overflow-hidden rounded-2xl border border-border/50 bg-gradient-to-br from-card/80 via-card/70 to-card/90 p-6"
              >
                <div className="absolute -top-10 -right-10 h-28 w-28 rounded-full bg-primary/20 blur-2xl" />
                <p className="text-xs tracking-widest uppercase text-muted-foreground mb-3">{item.status}</p>
          ...
          >
            ...
            <div className="relative z-10">
              <motion.div
                ...
                className="w-20 h-20 mx-auto mb-6 rounded-full bg-gradient-to-br from-primary to-accent p-[2px]"
              >
              ...
              <p className="text-lg text-muted-foreground max-w-2xl mx-auto mb-8">
                Visión y creación de <span className="text-primary font-semibold">Edwin Oswaldo Castillo Trejo</span>. 
                  ...
                  <>
                    ...
                    <Link to="/docs">
                      <Button
                        size="lg"
                        variant="outline"
                        className="glass-crystal px-8 py-6 text-lg rounded-xl border-border/50 hover:border-primary/50"
                      >
              ...
              <p className="text-sm text-muted-foreground mt-6">
                ...
                <br />
                <span className="text-muted-foreground/60">Visión de Edwin Oswaldo Castillo Trejo</span>
              </p>
            </div>
          </motion.div>
        </div>
