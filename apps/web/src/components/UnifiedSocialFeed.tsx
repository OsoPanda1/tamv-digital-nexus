...
// -------------------------------------------------------------------
// Mock data (sustituir por API más adelante)
// -------------------------------------------------------------------
...
const glassBase =
  "bg-gradient-to-br from-[#2e156b77] via-[#26093544] to-[#113c5955] backdrop-blur-2xl " +
  "border border-purple-400/15 shadow-[0_4px_48px_#3c42ff33]";
const glassHover =
  "hover:scale-[1.035] hover:shadow-[0_0_32px_#ff72dd66] transition-all duration-300";

const UNIFIED_TOOLS: ToolConfig[] = [
...
const EXCLUSIVE_SECTIONS: SectionConfig[] = [
  {
    icon: GraduationCap,
    label: "Universidad TAMV",
  ...
  return (
    <main className="min-h-screen w-full bg-gradient-to-br from-[#141028] via-[#20134b] to-[#4e23aa] scrollbar-thin pb-20 relative">
      ...
      >
        <nav
          className="
            bg-gradient-to-br from-cyan-500/10 via-[#1f194966] to-violet-900/80
            px-4 md:px-10 py-4
            flex flex-row items-center justify-between gap-4
            border-b border-purple-400/10 rounded-b-3xl
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <span
              className="
                text-[1.6rem] md:text-[2rem] font-black
          ...
          <div className="hidden md:flex flex-1 justify-center">
            <input
              className="
                rounded-full px-6 py-2 text-sm md:text-base w-full max-w-md
          ...
          <div className="flex items-center gap-4">
            <Button
              className={`
                hidden md:inline-flex rounded-full px-6 md:px-7 py-2
                text-sm md:text-lg font-extrabold text-white
                bg-gradient-to-br from-cyan-400 to-purple-500 via-pink-500
                shadow-[0_0_40px_#b253ff99]
                hover:shadow-[0_0_100px_#ffe15aff]
            ...
            <img
              ...
              className="w-10 h-10 md:w-12 md:h-12 rounded-full border-4 border-cyan-500/30 shadow-xl object-cover"
              alt="Perfil actual"
              loading="lazy"
            />
          </div>
      ...
      {/* VIDEO HERO */}
      <section className="flex flex-col items-center justify-center max-w-6xl mx-auto pt-10 md:pt-12 pb-4 px-2">
        <motion.div
        ...
        >
          ...
          <div
            className="
              ...
              text-lg md:text-2xl font-extrabold text-white
        ...
        <p className="py-4 text-center max-w-2xl mx-auto font-semibold text-sm md:text-lg text-white/75">
          {mainVideo.description}
        </p>
      </section>

      {/* LÍNEAS DE VIDEOS DESTACADOS */}
      <section className="relative py-3 flex flex-col gap-4 items-center">
        <div className="flex gap-4 md:gap-6 w-full max-w-5xl overflow-x-auto px-2 md:px-0">
          {[...Array(4)].map((_, idx) => (
            <motion.div
              key={`destacado-${idx}`}
              ...
              className={`
                ...
                rounded-2xl shadow-quantum backdrop-blur-lg relative
                bg-gradient-to-br from-[#2c216b]/80 to-[#1c3579]/70
                overflow-hidden ${glassHover}
              `}
            >
              <img
                src={`https://picsum.photos/seed/vidline1_${idx}/320/180`}
                alt={`Video destacado #${idx + 1}`}
                className="w-full aspect-video object-cover opacity-85"
                loading="lazy"
              />
              <div
                className="
                  absolute bottom-2 left-2 text-xs md:text-sm
                  px-3 md:px-4 py-1 rounded-full
                  bg-black/45 shadow-lg font-bold text-white
                "
              >
                Destacado #{idx + 1}
              </div>
          ...
          {[...Array(4)].map((_, idx) => (
            <motion.div
              ...
              className={`
                min-w-[240px] md:min-w-[260px]
                rounded-2xl shadow-quantum backdrop-blur-lg relative
                bg-gradient-to-br from-[#194f63]/80 to-[#0a254b]/70
                overflow-hidden ${glassHover}
              `}
            >
              <img
                src={`https://picsum.photos/seed/vidline2_${800 + idx}/320/180`}
                alt={`Epic viral #${idx + 1}`}
                className="w-full aspect-video object-cover opacity-85"
                loading="lazy"
              />
              <div
                className="
                  absolute bottom-2 left-2 text-xs md:text-sm
                  px-3 md:px-4 py-1 rounded-full
                  bg-white/25 shadow font-bold text-white
                "
              >
                Epic Viral #{idx + 1}
              </div>
      ...
      {/* TOOLBAR CATEGORÍAS SOCIAL-FUTURE */}
      <motion.section
        initial={{ y: 32, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ delay: 0.6, duration: 1.1, type: "spring" }}
        className="py-3 flex flex-wrap gap-4 md:gap-6 justify-center items-center w-full max-w-7xl mx-auto px-4"
      >
        {UNIFIED_TOOLS.map((btn) => (
          <Button
            key={btn.text}
            className={`
              h-14 md:h-16 min-w-[11rem] md:min-w-[13rem]
              px-6 md:px-10 uppercase text-sm md:text-xl
              font-extrabold rounded-full shadow-lg
              bg-gradient-to-br ${btn.color}
              flex items-center gap-3 md:gap-4 justify-center
              ${glassHover}
            `}
          >
            <btn.icon className="w-6 h-6 md:w-8 md:h-8 -ml-1" strokeWidth={2.5} />
            {btn.text}
          </Button>
        ))}
      </motion.section>
      ...
      >
        <h2
          className="
            text-3xl md:text-4xl font-black text-center mb-8
            bg-gradient-to-r from-cyan-400 via-fuchsia-500 to-yellow-400
      ...
      <motion.section
        ...
        className="
          ...
          gap-8 md:gap-14 max-w-7xl mx-auto
        "
      >
        <Card className={`${glassBase} flex flex-col gap-3 items-center h-full shadow-xl`}>
          ...
          <div
            className="
              h-20 md:h-24 rounded-xl w-full
              bg-gradient-to-br from-cyan-800/30 to-blue-700/30
              flex items-center justify-center text-sm md:text-xl
            "
          >
            Player AI • Charts • Rooms
          </div>
        </Card>
      ...
      >
        {EXCLUSIVE_SECTIONS.map((sec) => (
          <motion.div
            key={sec.label}
            whileHover={{ scale: 1.045, boxShadow: "0 4px 34px #ffffff44" }}
            className={`
            ...
            `}
          >
            <sec.icon className="w-10 h-10 md:w-12 md:h-12 drop-shadow-[0_2px_8px_#e4dcff66]" />
